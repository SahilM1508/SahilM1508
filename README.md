...
**AWS | Kubernetes | Docker | Jenkins | CI/CD | Linux | DevSecOps**
...
I'm an **Aspiring Cloud & DevOps Engineer** with hands-on experience in AWS Cloud, containerization, Kubernetes, CI/CD automation, Linux, and cloud-native application deployment.

I enjoy building **scalable, automated, and secure cloud infrastructure** using AWS, Docker, Kubernetes, Jenkins, and DevOps tools.

### 🔎 Areas of Interest

* ☁️ AWS Cloud & Cloud-Native Architecture
...
`EC2` `ECS Fargate` `EKS` `S3` `ECR` `IAM` `VPC` `ALB`

`Lambda` `API Gateway` `DynamoDB` `RDS` `Athena` `CloudWatch`
...
`Docker` `Kubernetes` `Amazon ECS` `Amazon EKS` `Helm`

### 🔐 DevSecOps

`SonarQube` `Trivy` `OWASP Dependency-Check` `Docker Scout`
...
`Prometheus` `Grafana` `CloudWatch` `Fluentd`

### 💻 Programming

`Python` `Java` `Node.js` `JavaScript` `Bash`
...
My projects cover the complete application delivery lifecycle:
...
* 🏆 **AWS Training & Certification Badges – 10 Specialized Courses**

### AWS Training Areas

`Amazon EKS` `GitOps` `Microservices` `Observability`

`Event-Driven Architecture` `AWS Control Tower`

`Serverless Security` `AWS Networking` `AppSync`

`Amazon Q Developer` `Autoscaling & Cost Optimization`
...
* Gained hands-on exposure to AWS Cloud.
* Worked with cloud storage and security concepts.
* Worked with IAM policies.
* Explored cloud monitoring.
* Gained exposure to multi-cloud architecture.
...
<p align="center">
  <b>☁️ Build • Automate • Secure • Deploy • Monitor 🚀</b>
</p>
